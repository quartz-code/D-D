...
def собрать(cfg: dict) -> str:
    ...
    строки: list[str] = [
        "# " + t("report.title"),
        "",
        f'- {t("report.started")}: {данные.get("начата") or "—"}',
        f'- {t("report.last_event")}: {данные.get("обновлено") or "—"}',
        f'- {t("report.stage")}: {данные.get("этап") or "—"}',
        f'- {t("report.attitude")}: {данные.get("отношение") or "—"}',
        f'- {t("report.messages")}: {данные.get("сообщений_израсходовано", 0)}',
        "- " + t("report.tokens", запрос=данные.get("токенов_запрос", 0),
                 ответ=данные.get("токенов_ответ", 0),
                 prompt=данные.get("токенов_запрос", 0),
                 completion=данные.get("токенов_ответ", 0)),
        "",
    ]
    ...
    строки += [
        ...
        "",
        f'- {t("report.commands")}: '
        f'{sum(1 for к in команды if к.get("вид") == "команда")}',
        f'- {t("report.replies")}: {len(переписка)}',
        f'- {t("report.confirmed")}: {len(подтверждённые)}',
        f'- {t("report.injections")}: {len(взломы)}',
        "",
    ]

    if подтверждённые:
        строки += ["## " + t("report.applied"), ""]
    ...
    for к in команды:
        if к.get("вид") != "команда":
            continue
        отметка = {"сценарная": t("report.mark.canned"),
                   "отклонена": t("report.mark.denied"),
                   "служебная": t("report.mark.service")}.get(к.get("тип", ""), "")
        хвост = f" ({отметка})" if отметка else ""
        лента.append((к.get("время", ""), f"`{к.get('команда', '')}`{хвост}"))
    for р in переписка:
        кто = t("report.players") if р.get("роль") == "игрок" else t("report.speaker")
        текст = str(р.get("текст", "")).replace("\n", " ")
        лента.append((р.get("время", ""), f"**{кто}:** {текст}"))
    for с in события:
        if с.get("тип") == "этап":
            лента.append((с.get("время", ""),
                          "_" + t("report.stage_line", этап=с.get("этап"),
                                  stage=с.get("этап")) + "_"))
        elif с.get("тип") == "действие_подтверждено":
            лента.append((с.get("время", ""),
                          "_" + t("report.applied_line", комната=с.get("комната"),
                                  действие=с.get("действие"), room=с.get("комната"),
                                  action=с.get("действие")) + "_"))
        elif с.get("тип") == "попытка_взлома":
            лента.append((с.get("время", ""),
                          "_" + t("report.injection_line", вид=с.get("вид"),
                                  kind=с.get("вид")) + "_"))

    if лента:
        строки += ["## " + t("report.timeline"), ""]
        for время, текст in sorted(лента, key=lambda п: п[0]):
            строки.append(f"- `{время}` {текст}")
        строки.append("")

    if not команды:
        строки += ["> " + t("report.empty_journal"), ""]

    строки += ["## " + t("report.features"), ""]
